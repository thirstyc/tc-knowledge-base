...
export function renderHead({ title, description, lang = 'en', assetPrefix = '', alternates = [], jsonLd = [], canonical = null } = {}) {
  const alternateLinks = alternates
    .map(({ hreflang, href }) => `\n<link rel="alternate" hreflang="${hreflang}" href="${href}" />`)
    .join('');
  // Self-referencing canonical: the page's own absolute URL, taken from its
  // hreflang alternate for this language unless the caller passes one. It
  // tells search engines which URL to index when the same page is reachable
  // more than one way (index.html vs /, query strings, ...).
  const canonicalHref = canonical ?? alternates.find((a) => a.hreflang === lang)?.href ?? null;
...
<link rel="stylesheet" href="${assetPrefix}styles.css" />${canonicalLink}${alternateLinks}${jsonLdScripts}
</head>`;
}
...
// Every footer link used to be href="#" except the mailto: The App, Tastings,
// The Writing, Instagram, Contact, Privacy Policy, Your Privacy Choices and
// Terms & Conditions -- 22,785 dead anchors across 2,930 pages, and a reader
// who clicked "Privacy Policy" stayed exactly where they were. Only two had a
// destination that actually exists, so the rest are gone rather than left
// pretending. Add a column back when there is something to point it at.
const APP_STORE_URL = 'https://apps.apple.com/ca/app/thirsty-cellar/id6772614506';
...
export function renderFooter({ lang = 'en', assetPrefix = '', frHref = null, enHref = null } = {}) {
  ...
  <script src="${assetPrefix}site.js"></script>`
    : `  <footer class="site-footer">
    <div class="footer-grid">
      <div class="footer-brand">
        <img src="${assetPrefix}assets/tc-monogram-white.png" alt="Thirsty Cunt" />
      ...
      <div class="footer-col">
        <p class="k">Explore</p>
        <div class="links">
          <a href="${APP_STORE_URL}">The App</a>
        </div>
      </div>
      <div class="footer-col">
...
// deriveCardTitle() in the old browser client; kept in sync by hand since that
...
// French sets a space inside guillemets (« comme ceci »), and a plain space
// there lets a line break strand the closing » at the start of a line. Every
// space around a guillemet becomes non-breaking (U+00A0) -- including the
// narrow U+202F typesetters prefer, which the body font can't be relied on to
// carry -- and the rare unspaced «pair» gets one added. Applied to a finished
// page, so every text path (hero, facts, answers, related cards, meta tags,
// JSON-LD) is covered in one place; English pages pass through untouched.
export function frenchSpacing(html, lang) {
...
// the old browser client; kept in sync by hand since that copy ran in the
...
// A meta description for an answer page.
//
// It used to be the first two sentences of the answer alone, which on a thin
// answer meant "Grilled meat. Lamb." -- nineteen characters, shared verbatim
// with nineteen other pairing pages. Twenty URLs describing themselves
// identically is a duplicate-content signal, and it tells a reader nothing
// about which one to click.
//
// Leading with the question fixes both at once: every question is unique, so
// every description is, and a short answer stops producing a description too
// short to be worth showing. It costs a little repetition against the title,
// which is a fair trade for a snippet that says what page this is.
//
// Whole sentences only, up to MAX. One sentence longer than MAX on its own is
// cut at a word boundary -- better a clean ellipsis than a word sliced in half.
const META_DESCRIPTION_MAX = 155;

export function metaDescription(question, answer, max = META_DESCRIPTION_MAX) {
  const q = question.replace(/\s+/g, ' ').trim();
  const sentences = answer.replace(/\s+/g, ' ').trim().split(/(?<=[.!?])\s+/).filter(Boolean);
  ...
  if (out === q && sentences.length > 0) {
    // The question alone left no room for even the first sentence.
    const room = max - q.length - 2;
    if (room > 20) {
      const cut = sentences[0].slice(0, room);
      out = `${q} ${cut.slice(0, cut.lastIndexOf(' '))}…`;
    }
  }
  return out.length > max ? `${out.slice(0, out.slice(0, max).lastIndexOf(' '))}…` : out;
}
