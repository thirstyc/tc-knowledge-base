...
// disk. Answer pages are generated from knowledge_chunks by
...
function answerPages(rootDir) {
  return readdirSync(rootDir)
    .filter((name) => /^answer-[a-z0-9-]+\.html$/.test(name))
    .sort();
}

export function writeSitemap(rootDir = '.') {
  const paths = [
    ...new Set([...STATIC_PAGES, ...TOPICS.map((t) => `topic-${t.slug}.html`), ...answerPages(rootDir)]),
  ];
  const urls = paths
