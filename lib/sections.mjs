...
export function sectionAnchor(row) {
  // A row read from content/docs carries the anchor exactly as the page
  // already published it. Prefer it: slugify(section_title) and
  // slugify(heading) are not the same string on every document, and an anchor
  // that shifts silently breaks the links regions.html points at it.
