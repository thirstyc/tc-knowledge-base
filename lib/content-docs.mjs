  ...
  for (const file of readdirSync(dir).filter((f) => f.endsWith('.md')).sort()) {
    ...
    for (const { heading, anchor, body } of doc.sections) {
      rows.push({
        ...
        anchor,
        // Overview keeps its parseOverview() layout; a section's content opens
        // with its heading line, which splitHeading() strips back off.
        content: heading === 'Overview' ? body : `${heading}\n${body}`,
        lang,
        file: full,
      });
    }
