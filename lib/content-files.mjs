...
const FIELDS = ['source_doc', 'chunk_type', 'section_title'];

export function serializeAnswer({ source_doc, chunk_type, section_title, question, answer }) {
  const front = FIELDS.map((key) => {
    const value = { source_doc, chunk_type, section_title }[key];
...
export function readAnswerRows() {
  const rows = [];
