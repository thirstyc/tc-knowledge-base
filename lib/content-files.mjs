...
// A row's `content` is "question answer" rejoined, so splitting it back is the
// same cut parseAnswer() made at the blank line. Shared because two separate
// things measure it -- content-generator picks what to expand or translate,
// audit-links reports the backlog -- and a thin answer has to mean the same
// number in both, or the audit reports work the generator won't do.
export function answerParts(row) {
  const i = row.content.indexOf('? ');
  return i === -1
    ? { question: row.content, answer: '' }
    : { question: row.content.slice(0, i + 1), answer: row.content.slice(i + 2) };
}

export function answerWords(row) {
  return answerParts(row).answer.split(/\s+/).filter(Boolean).length;
}
