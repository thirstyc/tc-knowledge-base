...
const ANSWER_GENERATOR_MARKER = 'Generated by generate-missing-pages.mjs';
...
function renderStub(target) {
  ...
  return `<!DOCTYPE html>
${REDIRECT_MARKER}
<html lang="${target.startsWith('fr/') ? 'fr' : 'en'}">
<head>
<meta charset="utf-8" />
<title>Moved</title>
...
for (const [from, to] of Object.entries(REDIRECTS)) {
  ...
  // Never clobber a hand-authored page. Generated answer pages are fine to
  // replace: listing one in REDIRECTS is how it gets retired (see config).
  const existing = existsSync(from) ? readFileSync(from, 'utf8') : null;
  if (existing && !existing.includes(REDIRECT_MARKER) && !existing.includes(ANSWER_GENERATOR_MARKER)) {
