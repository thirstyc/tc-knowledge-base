...
import { readAnswerRows, answerFilePath, CONTENT_DIR } from '../lib/content-files.mjs';
import {
  renderHeroBand,
  renderHead,
  renderHeader,
...
// hreflang only claims a translation that exists. An answer written in English
// with no French counterpart yet used to get an <link hreflang="fr"> and a
// footer link to fr/<slug>.html regardless -- 33 of them after the new grape
// answers landed, each one a link to a 404 and an hreflang pointing at
// nothing. A missing pair is a gap to fill, not a link to fabricate.
function hasFrenchPair(sourceDoc) {
  return existsSync(answerFilePath('fr', sourceDoc));
}

function hreflangAlternates(path, hasFr) {
  return [
    { hreflang: 'en', href: `${BASE_URL}/${path}` },
    ...(hasFr ? [{ hreflang: 'fr', href: `${BASE_URL}/fr/${path}` }] : []),
...
function renderAnswerPage(row, related, lang) {
  ...
  // A French page always has an English one; the reverse is not guaranteed.
  const hasFr = isFr || hasFrenchPair(row.source_doc);
  ...
  return [
    renderHead({
      ...
      lang,
      assetPrefix: ap,
      alternates: hreflangAlternates(slugPath, hasFr),
    ...
    renderHeader({
      ...
      lang,
      assetPrefix: ap,
      frHref: isFr || !hasFr ? null : `fr/${slugPath}`,
