...
|---|---|---|
| Read Obsidian vault | ⚠️ Partial | Two vault paths, now read from one place. [add-intake.mjs:6](add-intake.mjs#L6) and [process-knowledge-intake.mjs:10](process-knowledge-intake.mjs#L10) use `.../Thirsty Cunt/Knowledge Base`; [scripts/content-generator.mjs:49](scripts/content-generator.mjs#L49) (`--mode=vault-scan`) reads that one *and* `.../Thirsty Cunt/AI sommelier`. The third script that used to read the second path on its own, `process-obsidian-to-supabase.js`, was deleted on 2026-09-23 — it carried a service-role key, upserted to `knowledge_chunks` on a conflict target with no matching unique index (so it could not have run successfully), and duplicated what vault-scan already does. Both paths are absolute and local to one machine, so none of this runs in CI. |
