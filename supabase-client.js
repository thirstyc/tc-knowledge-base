...
const TOPIC_KEYWORDS = [
  ...
  { pattern: /(?:natural wines?|low-intervention)|(?:vins? nature(?:ls?|s)?\b|faible intervention|intervention minimale|peu interventionnistes?)/i, slug: 'natural-wine' },
  { pattern: /\bferment(?:ations?|ed|ing|s|[ée]e?s?)?(?![a-zA-ZÀ-ÖØ-öø-ÿ])/i, slug: 'fermentation' },
  { pattern: /sulf?ite/i, slug: 'sulphites' },
  { pattern: /(?:\btannins?\b)|(?:\btanins?\b)/i, slug: 'tannins' },
  { pattern: /(?:\b(?:un)?oak(?:ed|y)?\b|\bbarrels?\b|\bcasks?\b|\bbarriques?\b)|(?:\bch[êe]ne\b|\bbois[ée]e?s?(?![a-zA-ZÀ-ÖØ-öø-ÿ])|\bf[uû]ts?\b|\bbarriques?\b)/i, slug: 'oak' },
  { pattern: /(?:\bfault(s|y)?\b|(?<![-\w])corked\b|\btainted\b)|(?:(?<!par )d[ée]fauts?|\bd[ée]fectueu(?:x|se|ses)\b|\bbouchonn[ée]e?s?(?![a-zA-ZÀ-ÖØ-öø-ÿ]))/i, slug: 'faults' },
  { pattern: /(?:\bsparkling\b|\bchampagnes?\b|\bprosecco\b|\bcava\b|\bcr[ée]mant\b|\blambrusco\b|\bfranciacorta\b|\bp[ée]t[- ]nat\b|\bfizz(?:y|es)?\b|\bfrizzante\b|\bspritz(?:y)?\b)|(?:\bmousseux\b|\beffervescents?\b|\bp[ée]tillants?\b|\bchampagnes?\b|\bprosecco\b|\bcava\b|\bcr[ée]mant\b|\blambrusco\b)/i, slug: 'sparkling' },
